...
POST /auth/signup-invite — member registers using an invite token
"""
...
async def check_setup(db: AsyncSession = Depends(get_db)):
    existing = await db.scalar(select(User).where(User.role == "super_admin"))
...
async def invite_user(
    ...
    # Dynamic frontend URL from request origin
    frontend_url = request.headers.get("origin", "").rstrip("/")
